...
from message_broker import MessageBroker, build_message_broker
...
class DefaultDispatcher(MessageDispatcher):
    ...
    def __init__(self, endpoint_url: str):
        self.sender = RestApiSender(endpoint_url)

    def dispatch(self, message: str) -> None:
        self.sender.send(message)


class QueueDispatcher(MessageDispatcher):
    ...
    def dispatch(self, message: str) -> None:
        self.broker.publish(self.queue_name, message)


class MultiQueueDispatcher(MessageDispatcher):

    def __init__(self, broker: MessageBroker, queues: list[str],
                 exchange_name: str):
        self.broker = broker
        self.queues = queues
    ...
    def dispatch(self, message: str) -> None:
        self.broker.safe_publish(self.queues, self.exchange_name, message)
...
class DispatcherFacade:
    ...
    def set_strategy(self, dispatcher: MessageDispatcher):
        self.dispatcher = dispatcher
...
from abc import ABC, abstractmethod
from message_dispatcher import RestApiSender
